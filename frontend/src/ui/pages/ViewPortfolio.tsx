...
const ICONS = {
  star: FaStar, heart: FaHeart, check: FaCheckCircle, lightbulb: FaLightbulb, 
  music: FaMusic, camera: FaCamera, envelope: FaEnvelope, phone: FaPhone, 
  location: FaMapMarkerAlt, graduation: FaGraduationCap, briefcase: FaBriefcase,
};
...
export default function ViewPortfolio() {
  ...
  useEffect(() => {
    fetch(`https://onlineportfolio-4i6c.onrender.com/api/portfolios/view/${id}`)
      ...
      .then(data => {
        setPortfolio(data);
        // ตรวจสอบโครงสร้างข้อมูล pages/elements
        if (data.pages && data.pages.length > 0) {
          setPages(data.pages);
        } else if (data.elements) {
          setPages(Array.isArray(data.elements[0]) ? data.elements : [data.elements]);
        }
      })
  ...
  const renderShape = (el: any) => {
    ...
    // 🔥 1. แปลงชื่อ Type เป็นพิมพ์เล็กเสมอ ป้องกันบั๊กหาชื่อไม่เจอ
    ...
    if (type === 'circle') return <div style={{ ...common, borderRadius: '50%' }} />;
    
    // 🔥 2. อัปเดตการวาดเส้น (Line) 
    if (type === 'line') return <div style={{ width: '100%', height: '100%', backgroundColor: s.backgroundColor, opacity: s.opacity ?? 1 }} />;
    
    // 🔥 3. เพิ่มการวาดเส้นแบ่ง (Divider) ที่เคยล่องหนหายไป
    if (type === 'divider') {
    ...
    if (type === 'image') {
      return <img src={el.content} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', borderRadius: s.borderRadius || 0, opacity: s.opacity ?? 1, boxShadow: s.boxShadow }} />;
    }

    // 🔥 4. เพิ่มการรองรับ Icons
    if (type === 'icon') {
    ...
    // 🔥 Fallback ถ้าเป็นรูปทรงแบบอื่น จะให้โชว์กล่องเปล่าๆ แทนการหายไปเลย
    return <div style={common} />;
  };

  if (error) return <div className="h-screen flex items-center justify-center text-red-500 text-2xl font-bold">{error}</div>;
  if (!portfolio) return <div className="h-screen flex items-center justify-center">Loading...</div>;

  const currentElements = pages[currentPageIndex] || [];
  const currentCanvasBg = portfolio.page_backgrounds?.[currentPageIndex] || portfolio.backgroundColor || '#ffffff';

  return (
    <div className="min-h-screen bg-gray-200 py-10 flex flex-col items-center gap-8 overflow-auto">
      <style>{fontStyles}</style>
      <h1 className="text-3xl font-bold text-gray-800 bg-white px-6 py-2 rounded-full shadow-sm">{portfolio.title}</h1>
      ...
      >
        <div style={{ 
          ...
          {Array.isArray(currentElements) && currentElements.map((el: any) => (
             <div 
                key={el.id} 
                style={{ 
                  position: 'absolute', 
                  // บังคับเติม px ให้กับตำแหน่งและขนาด
                  left: typeof el.style.x === 'number' ? `${el.style.x}px` : el.style.x, 
                  top: typeof el.style.y === 'number' ? `${el.style.y}px` : el.style.y, 
                  width: typeof el.style.width === 'number' ? `${el.style.width}px` : el.style.width, 
                  height: typeof el.style.height === 'number' ? `${el.style.height}px` : el.style.height, 
                  transform: `rotate(${el.style.rotation || 0}deg)`, 
                  // ใช้ค่า zIndex จากฐานข้อมูล ถ้าไม่มีให้เป็น 1
      ...
      {pages.length > 1 && (
        <div className="flex justify-center items-center gap-4 p-2 bg-white rounded-xl shadow-lg border border-gray-200">
          <button className="btn btn-sm btn-circle btn-ghost" disabled={currentPageIndex === 0} onClick={() => setCurrentPageIndex(p => p - 1)}>❮</button>
          <span className="text-sm font-bold text-gray-700">Page {currentPageIndex + 1} / {pages.length}</span>
          <button className="btn btn-sm btn-circle btn-ghost" disabled={currentPageIndex === pages.length - 1} onClick={() => setCurrentPageIndex(p => p + 1)}>❯</button>
        </div>
      )}
    </div>
  );
