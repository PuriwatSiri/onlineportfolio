...
// Import Actions และ Type จาก Slice ที่เชื่อม API แล้ว
import { 
  fetchPackages, 
  addPackageAsync, 
  updatePackageAsync, 
...
// --- Component: Package List (ตารางแสดงข้อมูล) ---
function PackageList({ items, onAdd, onEdit }: { 
    items: Package[], 
    onAdd: () => void, 
    onEdit: (pkg: Package) => void 
}) {
  // 🔥 1. State สำหรับ Search และ Pagination
  const [search, setSearch] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 7;

  // 🔥 2. กรองข้อมูลตามที่ค้นหา
  ...
  return (
    <div>
      {/* Header พร้อม Search และปุ่ม Add */}
          ...
          <div className="relative">
            <input
              ...
              onChange={(e) => { 
                ...
                setCurrentPage(1); // พิมพ์ค้นหาให้กลับไปหน้า 1
              }}
              className="input input-bordered w-64"
            />
          </div>
          <button className="btn bg-gray-800 text-white hover:bg-gray-700" onClick={onAdd}>
            Add +
          </button>
        </div>
      </div>

      <div className="mb-2 text-sm text-gray-600 font-bold">Total Packages: {filteredItems.length}</div>
            ...
            {displayedItems.length > 0 ? (
              displayedItems.map((p) => (
                ...
                >
                  ...
                  <td className="font-bold text-green-600">฿{p.price.toLocaleString()}</td>
                  <td className="truncate max-w-xs text-gray-500">{p.package_detail}</td>
                </tr>
              ))
            ) : (
              <tr>
...
// --- Component: Package Form (ฟอร์มสร้าง/แก้ไข) ---
function PackageForm({ pkg, onBack, onSave, onDelete }: { 
    pkg: Package | null, 
    onBack: () => void, 
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <h1 className="text-2xl font-semibold mb-6">{isEdit ? 'Edit Package' : 'Create New Package'}</h1>
      <div className="p-6 border rounded-lg shadow-lg bg-white">
        ...
        <div className="flex justify-between pt-4 border-t mt-4">
          {isEdit ? (
            <button className="btn btn-error text-white" onClick={handleDeleteClick}>
              Delete Package
            </button>
          ) : (
            <div/>
          ...
          <div className="space-x-2">
            ...
            <button className="btn bg-gray-800 text-white hover:bg-gray-700" onClick={handleSubmit}>
  ...
  const handleSave = async (pkg: Package) => {
    try {
      if (editingPackage && (pkg._id || pkg.id)) {
        await dispatch(updatePackageAsync({ id: pkg._id || pkg.id, data: pkg })).unwrap();
  ...
  return (
    <div className="p-6">
      {mode === 'list' ? (
        ...
        <PackageForm pkg={editingPackage} onBack={() => setMode('list')} onSave={handleSave} onDelete={handleDelete} />
      )}
    </div>
  );
}