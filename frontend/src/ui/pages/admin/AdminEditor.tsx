...
const AdminEditor: React.FC = () => {
  ...
  const existingTemplate = useAppSelector((s) =>
    s.templates.items.find(
      (t) => t.id === templateIdFromUrl || t._id === templateIdFromUrl,
    ),
  );
  ...
  const [pageBackgrounds, setPageBackgrounds] = useState<string[]>(() => {
    if (
      existingTemplate?.page_backgrounds &&
      existingTemplate.page_backgrounds.length > 0
    ) {
      return existingTemplate.page_backgrounds;
    }
    if (existingTemplate?.pages)
      return new Array(existingTemplate.pages.length).fill("#ffffff");
    return ["#ffffff"];
  });
  ...
  useEffect(() => {
    if (existingTemplate) {
      ...
      setTemplateActive(
        existingTemplate.active !== undefined ? existingTemplate.active : true,
      );
    }
  }, [existingTemplate]);
  ...
  const applyTemplate = useCallback(
    (tplId: string) => {
      ...
      if (tpl.pages && tpl.pages.length > 0) {
        ...
        newPagesToAdd = [
          tpl.elements.map((e: any) => ({ ...e, id: uuidv4() })),
        ];
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (existingTemplate?._id || existingTemplate?.id) {
        ...
        await dispatch(
          updateTemplateAsync({ id, data: payloadToSend }),
        ).unwrap();
          ...
          <div className="flex flex-row gap-3 w-full xl:w-auto shrink-0 overflow-x-auto pb-1 xl:pb-0">
            ...
            <div className="flex flex-col gap-0.5 flex-1 xl:flex-none min-w-[120px]">
              ...
              <select
                ...
                onChange={(e) =>
                  setTemplateActive(e.target.value === "Published")
                }
                className={`select select-sm select-bordered w-full xl:w-40 text-xs font-medium rounded-lg h-9 min-h-0 ${
                  templateActive
              ...
              <div className="flex gap-1">
                ...
                <button
                  ...
                  onClick={() => {
                    setPages((p) => {
                      const np = [...p];
                      np[currentPageIndex] = np[currentPageIndex].filter(
                        (e) => e.id !== selectedId,
                      );
                      return np;
                    });
