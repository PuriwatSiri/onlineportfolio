...
import { fetchIssues, updateIssueAsync, Issue } from '@/store/slices/issuesSlice';

function ReportList({ items, onSelectReport }: { items: Issue[], onSelectReport: (issue: Issue) => void }) {
  ...
  return (
    <div>
      ...
      <div className="mb-2 text-sm text-gray-600 font-bold">Total Reports: {filteredItems.length}</div>
      
      <div className="overflow-x-auto bg-white rounded-lg shadow border">
        <table className="table w-full">
          {/* 👇 1. เติม bg-gray-50 ให้หัวตาราง เพื่อให้สีและธีมเหมือนหน้า Users */}
          ...
          <tbody>
            {displayedItems.length > 0 ? (
              displayedItems.map((issue: any) => {
                const fullName = `${issue.user_id?.firstname || ''} ${issue.user_id?.lastname || ''}`.trim();
                
                return (
                  <tr key={issue._id} className="hover:bg-gray-50 border-b-gray-100">
                    {/* 👇 2. เอา text-sm ออก เพื่อให้ฟอนต์กลับมาขนาดปกติเท่าตารางอื่น */}
                    ...
                    {/* 👇 3. เอา text-sm ออกเช่นกัน */}
                    <td className="text-gray-600">
                      {new Date(issue.report_date || issue.reportDate || issue.createdAt).toLocaleDateString()}
                    </td>
                    
                    <td className="text-center space-x-2">
                      <button className="btn btn-xs bg-gray-800 text-white hover:bg-gray-700" onClick={() => onSelectReport(issue)}>
                        View
                      </button>
                    </td>
                  </tr>
...
function ReportDetailForm({ issue, onBack, onSave }: { issue: Issue, onBack: () => void, onSave: (issue: Issue) => void }) {
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <div className="p-8 border rounded-lg shadow-lg bg-white">
        <div className="text-xl font-bold mb-4 border-b pb-2">{issue.user_id?.firstname} {issue.user_id?.lastname}</div> 
        ...
        <div className="grid grid-cols-3 gap-y-4 text-md mb-4 mt-4">
          ...
          <div className="col-span-2 font-bold">{formData.category || 'System'}</div>
          <div className="font-medium text-gray-500 flex items-center">Status</div>
          <div className="col-span-2 font-bold">
            <select name="status" value={formData.status} onChange={handleChange} className="select select-bordered w-full max-w-xs">
          ...
          <div className="col-span-2 font-normal whitespace-normal bg-gray-50 p-3 rounded border">{formData.description}</div>
        </div>

        <div className="form-control mt-6">
            <label className="label"><span className="label-text font-bold text-gray-700">Admin Note</span></label>
            <textarea name="note" value={formData.note || ''} onChange={handleChange}  className="textarea textarea-bordered h-24 w-full"></textarea>
        </div>
        
        <div className="flex justify-end mt-8 pt-4 border-t space-x-2">
          <button className="btn btn-ghost" onClick={onBack}>Cancel</button>
          <button className="btn bg-gray-800 text-white hover:bg-gray-700" onClick={() => onSave(formData)}>Save Changes</button>
        </div>
      </div>
    </div>
  )
...
export default function AdminIssues() {
  ...
  return (
    <div className="p-6">
      {selectedIssue ? (
        <ReportDetailForm issue={selectedIssue} onBack={() => setSelectedIssue(null)} onSave={handleSave} />
      ) : (
        <ReportList 
          items={items} onSelectReport={(u) => setSelectedIssue(u)} />
      )}
    </div>
  );
}