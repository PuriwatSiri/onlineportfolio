...
export default function AdminPayments() {
  ...
  const handleApprove = async (paymentId: string) => {
    if (!confirm('Are you sure you want to approve this slip and add time to the user\'s package?')) return;
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-2 text-sm text-gray-600 font-bold">Total Payments: {filteredPayments.length}</div>
          ...
          <tbody>
            {displayedPayments.length > 0 ? (
              displayedPayments.map(p => {
                const userObj = typeof p.user_id === 'object' ? p.user_id : null;
                ...
                return (
                  <tr key={p._id} className="hover:bg-gray-50">
                    ...
                    <td>{new Date(dateToFormat).toLocaleString('en-GB', {
                        day: '2-digit',
                        month: '2-digit',
                        year: 'numeric',
                        hour: '2-digit',
                    ...
                    <td>
                      {userObj ? (
                          <><div className="font-bold">{userObj.firstname} {userObj.lastname}</div><div className="text-xs text-gray-500">{userObj.email}</div></>
                      ) : 'User ID: ' + p.user_id}
                    </td>
                    ...
                    <td>
                      <span className={`badge ${p.status === 'approved' ? 'badge-success text-white' : p.status === 'rejected' ? 'badge-error text-white' : 'badge-warning'}`}>
                    ...
                    <td className="text-center">
                      {p.status === 'pending' ? (
                        <div className="flex gap-2 justify-center">
                          <button onClick={() => handleApprove(p._id)} className="btn btn-xs btn-success text-white">
                            Approve
                          </button>
                          <button onClick={() => handleReject(p._id)} className="btn btn-xs btn-error text-white">
                            Reject
                          </button>
                        </div>
                      ) : (
                        <span className="text-gray-400 text-sm font-semibold">Reviewed</span>
                      )}
                    </td>
                  </tr>
                )
