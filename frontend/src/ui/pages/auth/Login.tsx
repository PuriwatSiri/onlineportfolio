...
const onSubmit = async (data: Form) => {
  try {
    const response = await fetch('https://onlineportfolio-4i6c.onrender.com/api/users/login', {
      method: 'POST',
      ...
      <div className="max-w-md w-full bg-white p-10 rounded-lg shadow-xl border border-gray-200">
        <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="pt-2 space-y-3">
            ...
            <Link to="/register" className="btn btn-neutral w-full">Register</Link>
          </div>
        </form>

        <div className="mt-5 text-right">
          <Link to="/forgot" className="text-sm text-red-600 hover:text-red-700 font-medium">
