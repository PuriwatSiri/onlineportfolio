...
export default function PackagesPage() {
  ...
  const handleSelectPackage = (pkgId: string) => {
    ...
    if (pendingPayment) {
      alert(
        "You have a pending payment already. Please wait for confirmation.",
      );
  ...
  const currentPlan =
    items.find((p) => (p.id || p._id) === user?.packageId) ||
    ...
    <div className="p-8 bg-white min-h-screen">
        ...
        {currentPlan && (
          <div className="text-right">
            <p className="text-lg text-gray-700 font-medium">
              Current Plan : {currentPlan.package_name}
            </p>
            {expiryDate && (
              <p className="text-sm text-gray-500">Expired on : {expiryDate}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {items.map((p) => {
          ...
          const style =
            planStyles[p.package_name] || "bg-gray-50 border border-gray-200";
