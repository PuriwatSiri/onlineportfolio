    ...
    const fetchPortfolios = async () => {
      ...
      try {
        ...
        const res = await fetch(
          "https://onlineportfolio-4i6c.onrender.com/api/portfolios/me",
          {
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {templates
              ...
              .map((x) => (
                <div
                  key={x.id || x._id}
                  className="card bg-base-100 shadow card-hover rounded-lg overflow-hidden border border-gray-100"
                >
        ...
        {!isLoggedIn ? (
          <div className="flex flex-col items-center justify-center py-16 px-4 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50">
            <div className="text-center">
              ...
              <Link
                to="/login"
                className="btn bg-black text-white hover:bg-gray-800 px-8 border-none"
              >
                Login
              </Link>
            </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {myPortfolios.map((port) => (
                  <div
                    key={port._id}
                    className="card bg-base-100 shadow-sm hover:shadow-md transition border border-gray-200"
                  >
                    <figure className="h-40 bg-gray-100 overflow-hidden relative">
                      {port.cover_image ? (
                        ...
                        <div className="flex items-center justify-center h-full w-full text-gray-400 bg-gray-200">
                          No Image
                        </div>
                      )}
                    </figure>
